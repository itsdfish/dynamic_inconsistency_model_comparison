...
for df_subj ∈ groupby(df_stacked, :subject)
    ...
    estimator = qdim_uniform_model(data, parms)
    pt = pigeons(
    ...
    savefig(
        posterior_plots,
        "qdim_uniform_output/posterior_plots/subj_$(subj_id)_posterior_plots.png"
    )
    jldsave(
        "qdim_uniform_output/pigeons_output/subject_$(subj_id)_output.jld2";
        subj_id,
        chain,
        pt,
        data
