...
for df_subj ∈ groupby(df_stacked, :subject)
    ...
    estimator = priority_uniform_model(data)
    pt = pigeons(
    ...
    savefig(
        posterior_plots,
        "priority_uniform_model_output/posterior_plots/subj_$(subj_id)_posterior_plots.png"
    )
    jldsave(
        "priority_uniform_model_output/pigeons_output/subject_$(subj_id)_output.jld2";
        subj_id,
        chain,
        pt,
        data
