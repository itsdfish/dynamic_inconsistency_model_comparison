...
for df_subj ∈ groupby(df_stacked, :subject)
    ...
    data = format_data(df_subj, ReferencePointModel)
    estimator = reference_point_uniform_model(data, parms)
    pt = pigeons(
    ...
    savefig(
        posterior_plots,
        "reference_point_uniform_model_output/posterior_plots/subj_$(subj_id)_posterior_plots.png"
    )
    jldsave(
        "reference_point_uniform_model_output/pigeons_output/subject_$(subj_id)_output.jld2";
        subj_id,
        chain,
        pt,
        data
